...
You may wish to use ${chalk_1.default.green('ionic cordova prepare')} if you run your project with Android Studio or Xcode.
    ...
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const { getPlatforms, installPlatform } = yield Promise.resolve().then(() => require('@ionic/cli-utils/lib/integrations/cordova/project'));
            ...
            if (platform) {
                if (!platforms.includes(platform)) {
                    ...
                    if (confirm) {
                        yield installPlatform(this.env, platform, cordova.root);
                    }
                    else {
                        throw new errors_1.FatalException(`Can't prepare for ${chalk_1.default.green(platform)} unless the platform is installed.\n` +
            ...
            if (options.build) {
                ...
                const buildOptions = utils_1.generateBuildOptions(metadata, inputs, options);
                if (buildOptions['platform']) {
                    // TODO: use runner directly
                    yield build({ config: this.env.config, log: this.env.log, shell: this.env.shell, project: this.project }, inputs, buildOptions);
                }
                else {
